...
/**
 * 
 * @param {object} opts -- stay, join, updn
 */
function MergeXML(opts) {
  ...
  that.AddSource = function(xml) {
    if (typeof xml === 'object') {
      var doc = that.Get(1, xml) ? xml : false;
  ...
  var Load = function(src) {
    if (mde === 1) {
      if (that.dom) {
        var doc = psr.parseFromString(src, 'text/xml');
        var rlt = ParseError(doc) ? doc : false;
      } else {
    ...
    for (var i = 0; i < src.childNodes.length; i++) {
      ...
      if (node.nodeType === XML_ELEMENT_NODE) {
        ...
        if (obj === null || obj.namespaceURI !== node.namespaceURI) {
          ...
          var obj = that.Query(pth); /* destination parent */
  ...
  that.Query = function(qry) {
    ...
    try {
      if (mde === 1) {
        var rlt = xpe.evaluate(qry, that.dom, nsr, XPathResult.FIRST_ORDERED_NODE_TYPE, null);
